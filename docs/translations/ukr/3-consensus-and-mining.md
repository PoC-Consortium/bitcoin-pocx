...
- **Time Bended дедлайни:** Трансформація розподілу (експоненційний→Вейбулла, форма k=3) зменшує довгі блоки, покращує середній час блоків
...
**Реалізація:** `src/pocx/consensus/difficulty.cpp:GetNextGenerationSignature()` (викликається з `src/pocx/mining/block_context.cpp:GetNewBlockContext()`)
...
- Цільовий час блоку: 120 секунд (усі мережі)
...
**Призначення:** Трансформує експоненційний розподіл у розподіл Вейбулла (форма k=3). Дуже хороші рішення куються пізніше (мережа має час просканувати диски), погані рішення покращуються. Зменшує довгі блоки, підтримує середні 120с.
...
**Отримувач coinbase** (не забезпечується консенсусом):

Майнер встановлює вихід coinbase для оплати ефективному підписанту (`src/pocx/mining/block_builder.cpp:CreateCoinbaseScript()`), але це **не** перевіряється консенсусом. Консенсус забезпечує лише те, що *підпис блоку* створений ефективним підписантом — перевірка `bad-pocx-assignment-sig` вище. Правила `bad-pocx-coinbase` не існує; отримувача coinbase обирає майнер.
...
- Активуються після періоду затримки (за замовчуванням: 30 блоків; 4 на regtest)
