...
**Implementatie:** `src/pocx/assignments/opcodes.cpp`
...
for (const auto& tx : block.vtx) {
    for (const CTxOut& output : tx.vout) {
        if (IsAssignmentOpReturn(output)) {
            ...
            // Haal huidige toewijzing op - moet in ASSIGNED-status zijn om in te trekken
...
**Implementatie:** `src/validation.cpp:ConnectBlock()`
...
**Implementatie:** `src/pocx/assignments/opcodes.cpp:VerifyPlotOwnership()`
...
**Implementatie:** `src/consensus/tx_check.cpp`
...
**Implementatie:** `src/validation.cpp:PreChecks()`
...
**Implementatie:** `src/coins.cpp:Flush()`
...
**Implementatie:** `src/txdb.cpp:BatchWriteAssignments()`
...
**Implementatie:** `src/validation.cpp:DisconnectBlock()`
...
private:
    ...
    mutable ForgingAssignmentsMap deletedAssignments;  // Track deletions (map, not set)  // Houd verwijderingen bij
...
**Implementatie:** `src/pocx/rpc/assignments.cpp`
...
**Implementatie:** `src/pocx/rpc/assignments_wallet.cpp`
...
**Implementatie:** `src/pocx/assignments/transactions.cpp`
...
```
src/
├── coins.h                        # ForgingAssignment-struct, CCoinsViewCache-methoden
├── coins.cpp                      # Cachebeheer, batchschrijfacties
│
├── txdb.h                         # CCoinsViewDB toewijzingsmethoden
├── txdb.cpp                       # Database lezen/schrijven
│
...
└── pocx/
    ...
    │   ├── opcodes.h              # Markeringsdefinities, OP_RETURN-ops, eigenaarschapscontrole
    │   ├── assignment_state.h     # GetEffectiveSigner, GetAssignmentState-helpers
    │   ├── assignment_state.cpp   # Toewijzingsstatus-queryfuncties
    │   ├── replay.h               # Reorg/replay van toewijzingseffecten
    │   ├── replay.cpp             # ApplyAssignmentEffectsForReplay (reorg-pad)
    ...
    │
    └── consensus/
        └── params.h               # PoCXCompressionBounds, genesis base target, compressieschema
```

> **Opmerking:** De toewijzingsvertragingsconstanten `nForgingAssignmentDelay` / `nForgingRevocationDelay` bevinden zich in de `Consensus::Params` van Bitcoin Core (`src/consensus/params.h`) en worden per netwerk ingesteld in `src/kernel/chainparams.cpp` — niet in `pocx/consensus/params.h`.
...
- Integratietests: regtest-shellscripts onder `scripts/assignments/` en `scripts/mining/`
