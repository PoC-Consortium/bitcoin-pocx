...
- **מצב שרת כרייה**: דגל `` מאפשר תכונות הקשורות לכרייה
...
**סרגל כלים**: אייקון כרייה (נראה רק עם דגל ``)
...
- פלט OP_RETURN (סקריפט 46 בתים, מטען נתונים 44 בתים): סמן `POCX` + plot_address + forging_address
- פלט עודף: חוזר לארנק
...
**מבנה עסקה**:
- קלט: UTXO מכתובת plot (מוכיח בעלות)
- פלט OP_RETURN (סקריפט 26 בתים, מטען נתונים 24 בתים): סמן `XCOP` + plot_address
- פלט עודף: חוזר לארנק
...
**סיבה**: צומת הופעל ללא דגל ``
**פתרון**: הפעל מחדש עם `bitcoin-qt -server`
