...
(2) אלגוריתם Time-Bending, המשנה את התפלגות ה-deadline מאקספוננציאלית ל-Weibull (צורה k=3), מפחית שונות בזמני בלוקים מבלי לשנות את הממוצע;
...
Where `account_id` is the 20-byte HASH160 of the miner\'s public key. זה מייצר רצף של ערכי אנטרופיה חזקים קריפטוגרפית ותלויי-כורה. מכיוון שהמפתח הציבורי של כורה אינו ידוע עד שהבלוק הקודם מתפרסם, אף משתתף לא יכול לחזות בחירות scoop עתידיות. זה מונע חישוב מראש סלקטיבי או plotting אסטרטגי ומבטיח שכל בלוק מציג עבודת כרייה רעננה באמת.
...
| תקופה | שנים | Halvings | קנה מידה מינימלי | עבודת Plot (מול בסיס POC2) |
|--------|-------|----------|-------------|---------------------|
...
עמודת המכפיל מבוטאת ביחס לבסיס **POC2** הלא-מוקשח. מכיוון שפורמט X1 המוקשח כבר מטמיע פי 2 מהעבודה של POC2, רמה Xn שווה ל-2ⁿ × POC2 — באופן שקול 2^(n-1) × X1, התואם להגדרה לכל רמה בסעיף 3.5.
...
| סובסידיה ראשונית | 10 BTCX |
| מרווח halving | 1050000 בלוקים (~4 שנים) |
| היצע כולל | ~21 מיליון BTCX |
...
| מצב קיבולת נמוכה | מופעל (plots של ~16 MiB, 64 nonces) |
...
המערכת פעילה ברשת הראשית מאז 3 במאי 2026. כוח כרייה נגזר מקיבולת אחסון במקום קצב hash, מפחית צריכת אנרגיה בסדרי גודל תוך שמירה על המודל הכלכלי המוכח של Bitcoin.
