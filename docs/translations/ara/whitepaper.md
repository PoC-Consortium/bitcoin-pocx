...
(2) خوارزمية ثني الوقت، التي تحوّل توزيعات المواعيد النهائية من أسية إلى Weibull (shape k=3)، مما يقلل تباين وقت الكتلة دون تغيير المتوسط؛
...
يحافظ Bitcoin-PoCX على بنية Bitcoin Core من خلال تعديلات طفيفة موسومة بالميزات، مع عزل منطق PoC عن كود الإجماع الحالي. يحافظ النظام على السياسة النقدية لـ Bitcoin من خلال استهداف فاصل كتلة 120 ثانية وتعديل إعانة الكتلة إلى 10 BTCX. تُعوّض الإعانة المُخفّضة الزيادة بمقدار خمسة أضعاف في تردد الكتل، مما يُبقي معدل الإصدار طويل المدى متوافقاً مع جدول Bitcoin الأصلي ويحافظ على الحد الأقصى للعرض البالغ ~21 مليون.
...
Where `account_id` is the 20-byte HASH160 of the miner\'s public key. ينتج هذا سلسلة من قيم الإنتروبيا القوية تشفيرياً والمُعتمدة على المُعدّن. لأن المفتاح العام للمُعدّن غير معروف حتى نشر الكتلة السابقة، لا يمكن لأي مشارك التنبؤ باختيارات scoop المستقبلية. يمنع هذا الحساب المسبق الانتقائي أو الرسم الاستراتيجي ويضمن أن كل كتلة تُقدّم عمل تعدين جديداً حقيقياً.
...
| الفترة | السنوات | التنصيفات | الحد الأدنى للمقياس | عمل الرسم (مقابل خط أساس POC2) |
|--------|---------|-----------|---------------------|------------------|
| الحقبة 0 | 0-4 | 0 | X1 | 2× POC2 |
| الحقبة 1 | 4-12 | 1-2 | X2 | 4× POC2 |
| الحقبة 2 | 12-28 | 3-6 | X3 | 8× POC2 |
| الحقبة 3 | 28-60 | 7-14 | X4 | 16× POC2 |
| الحقبة 4 | 60-124 | 15-30 | X5 | 32× POC2 |
| الحقبة 5 | 124+ | 31+ | X6 | 64× POC2 |

عمود المُضاعف مُعبّر عنه نسبة إلى خط أساس **POC2** غير المُقوّى. بما أن صيغة X1 المُقوّاة تُضمّن بالفعل 2× عمل POC2، فإن المستوى Xn يساوي 2ⁿ × POC2 — أي ما يكافئ 2^(n-1) × X1، بما يطابق التعريف لكل مستوى في القسم 3.5.
...
| الإعانة الأولية | 10 BTCX |
| فاصل التنصيف | 1050000 كتلة (~4 سنوات) |
| إجمالي العرض | ~21 مليون BTCX |
...
| المعلمة | القيمة |
|---------|--------|
| البايتات السحرية | `0x6d 0xf2 0x48 0xb4` |
...
| تفعيل التعيين | 4 كتل |
| إلغاء التعيين | 8 كتل |
| وضع السعة المنخفضة | مُفعّل (~16 MiB رسومات، 64 nonces) |
...
النظام نشط على الشبكة الرئيسية منذ 3 مايو 2026. تُشتق قوة التعدين من سعة التخزين بدلاً من معدل التجزئة، مما يقلل استهلاك الطاقة بأوامر من الحجم مع الحفاظ على النموذج الاقتصادي المُثبت لـ Bitcoin.
