...
**Implementacja:** `src/pocx/assignments/opcodes.cpp`
...
for (const auto& tx : block.vtx) {
    for (const CTxOut& output : tx.vout) {
        if (IsAssignmentOpReturn(output)) {
            ...
            // Pobierz aktualny przydział — musi być w stanie ASSIGNED, aby odwołać
...
**Implementacja:** `src/validation.cpp:ConnectBlock()`
...
**Implementacja:** `src/pocx/assignments/opcodes.cpp:VerifyPlotOwnership()`
...
**Implementacja:** `src/consensus/tx_check.cpp`
...
**Implementacja:** `src/validation.cpp:PreChecks()`
...
**Implementacja:** `src/coins.cpp:Flush()`
...
**Implementacja:** `src/txdb.cpp:BatchWriteAssignments()`
...
**Implementacja:** `src/validation.cpp:DisconnectBlock()`
...
private:
    ...
    mutable ForgingAssignmentsMap deletedAssignments;  // Track deletions (map, not set)  // Śledź usunięcia
...
**Implementacja:** `src/pocx/rpc/assignments.cpp`
...
**Implementacja:** `src/pocx/rpc/assignments_wallet.cpp`
...
**Implementacja:** `src/pocx/assignments/transactions.cpp`
...
└── pocx/
    ...
    │   ├── replay.h               # Reorg/odtwarzanie efektów przydziałów
    │   ├── replay.cpp             # ApplyAssignmentEffectsForReplay (ścieżka reorg)
    ...
    │
    └── consensus/
        └── params.h               # PoCXCompressionBounds, genesis base target, harmonogram kompresji
```

> **Uwaga:** Stałe opóźnień przydziałów `nForgingAssignmentDelay` / `nForgingRevocationDelay` znajdują się w `Consensus::Params` Bitcoin Core (`src/consensus/params.h`) i są ustawiane per sieć w `src/kernel/chainparams.cpp` — nie w `pocx/consensus/params.h`.
...
- Testy jednostkowe: `src/test/pocx_tests.cpp`, `src/test/pocx_simd_tests.cpp`
- Testy integracyjne: skrypty powłoki regtest w `scripts/assignments/` i `scripts/mining/`
