...
**Implementation:** `src/pocx/assignments/opcodes.cpp`
...
for (const auto& tx : block.vtx) {
    for (const CTxOut& output : tx.vout) {
        if (IsAssignmentOpReturn(output)) {
            ...
            // Hämta aktuell tilldelning - måste vara i ASSIGNED-tillstånd för att återkalla
...
**Implementation:** `src/validation.cpp:ConnectBlock()`
...
**Implementation:** `src/pocx/assignments/opcodes.cpp:VerifyPlotOwnership()`
...
**Implementation:** `src/consensus/tx_check.cpp`
...
**Implementation:** `src/validation.cpp:PreChecks()`
...
**Implementation:** `src/coins.cpp:Flush()`
...
**Implementation:** `src/txdb.cpp:BatchWriteAssignments()`
...
**Implementation:** `src/validation.cpp:DisconnectBlock()`
...
private:
    ...
    mutable ForgingAssignmentsMap deletedAssignments;  // Track deletions (map, not set)  // Spåra borttagningar
...
**Implementation:** `src/pocx/rpc/assignments.cpp`
...
**Implementation:** `src/pocx/rpc/assignments_wallet.cpp`
...
**Implementation:** `src/pocx/assignments/transactions.cpp`
...
```
src/
├── coins.h                        # ForgingAssignment-struktur, CCoinsViewCache-metoder
├── coins.cpp                      # Cachehantering, batchskrivningar
│
├── txdb.h                         # CCoinsViewDB-tilldelningsmetoder
├── txdb.cpp                       # Databasläsning/skrivning
│
...
└── pocx/
    ...
    │   ├── opcodes.cpp            # Markördefinitioner, OP_RETURN-ops, ägarskapscheck
    ...
    │   ├── replay.h               # Reorg/återspelning av tilldelningseffekter
    │   ├── replay.cpp             # ApplyAssignmentEffectsForReplay (reorg-väg)
    ...
    │
    └── consensus/
        └── params.h               # PoCXCompressionBounds, genesis-basmål, kompressionsschema
```

> **Obs:** Tilldelningsfördröjningskonstanterna `nForgingAssignmentDelay` / `nForgingRevocationDelay` finns i Bitcoin Cores `Consensus::Params` (`src/consensus/params.h`) och sätts per nätverk i `src/kernel/chainparams.cpp` — inte i `pocx/consensus/params.h`.
...
- Enhetstester: `src/test/pocx_tests.cpp`, `src/test/pocx_simd_tests.cpp`
- Integrationstester: regtest-skalskript under `scripts/assignments/` och `scripts/mining/`
