...
(2) デッドライン分布を指数分布からWeibull分布（形状 k=3）に変換するタイムベンディングアルゴリズムにより、平均を変更せずにブロック時間の分散を削減;
...
Where `account_id` is the 20-byte HASH160 of the miner\'s public key. これは、暗号学的に強力なマイナー依存のエントロピー値のシーケンスを生成します。マイナーのaccount IDは前のブロックが公開されるまで不明であるため、参加者は将来のスクープ選択を予測できません。これにより、選択的な事前計算や戦略的なプロット作成が防止され、すべてのブロックが真に新鮮なマイニング作業を導入することが保証されます。
...
| 期間 | 年数 | 半減期 | 最小スケーリング | プロット作業（POC2ベースライン比） |
|--------|-------|----------|-------------|---------------------|
| エポック0 | 0-4 | 0 | X1 | 2× POC2 |
| エポック1 | 4-12 | 1-2 | X2 | 4× POC2 |
| エポック2 | 12-28 | 3-6 | X3 | 8× POC2 |
| エポック3 | 28-60 | 7-14 | X4 | 16× POC2 |
| エポック4 | 60-124 | 15-30 | X5 | 32× POC2 |
| エポック5 | 124以降 | 31以降 | X6 | 64× POC2 |

倍率の列は、ハード化されていない**POC2**ベースラインに対して表されます。ハード化されたX1形式はすでにPOC2の2倍の作業を埋め込んでいるため、レベルXnは 2ⁿ × POC2 に等しく — 同等に 2^(n-1) × X1 であり、第3.5節のレベルごとの定義と一致します。
...
システムは2026年5月3日よりメインネットで稼働しています。マイニングパワーはハッシュレートではなくストレージ容量から導出され、Bitcoinの実績ある経済モデルを維持しながらエネルギー消費を桁違いに削減します。
