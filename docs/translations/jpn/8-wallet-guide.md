...
6. `nForgingAssignmentDelay`ブロック後に割り当てがアクティブに:
   ...
   - Regtest: 4ブロック（120秒間隔で約8分）

**トランザクション手数料**: デフォルト10× `minRelayFee`（カスタマイズ可能）

**トランザクション構造**:
- 入力: プロットアドレスからのUTXO（所有権を証明）
- OP_RETURN出力（46バイトスクリプト、44バイトデータペイロード）: `POCX`マーカー + plot_address + forging_address
- お釣り出力: ウォレットに戻る
...
**トランザクション構造**:
- 入力: プロットアドレスからのUTXO（所有権を証明）
- OP_RETURN出力（26バイトスクリプト、24バイトデータペイロード）: `XCOP`マーカー + plot_address
- お釣り出力: ウォレットに戻る
