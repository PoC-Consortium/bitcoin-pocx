...
**Lösung**: Verteilungstransformation von exponentiell zu Weibull (shape k=3) mittels Kubikwurzel: `Y = scale × (X^(1/3))` where `X = raw_quality / base_target`.
...
- **Blockzeit**: 120 Sekunden (alle Netzwerke: Mainnet, Testnet, Regtest)
