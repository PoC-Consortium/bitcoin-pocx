...
- **Time Bended крайни срокове:** Трансформация на разпределението (експоненциално→Weibull (shape k=3)) намалява дългите блокове, подобрява средните времена на блокове
...
**Имплементация:** `src/pocx/consensus/difficulty.cpp:GetNextGenerationSignature()` (извиква се от `src/pocx/mining/block_context.cpp:GetNewBlockContext()`)
...
- Целево време на блок: 120 секунди (всички мрежи)
...
**Цел:** Трансформира експоненциално в Weibull разпределение (shape k=3). Много добрите решения се подписват по-късно (мрежата има време да сканира дисковете), лошите решения се подобряват. Намалява дългите блокове, поддържа средно 120s.
...
- Активират се след период на забавяне (по подразбиране: 30 блока; 4 на regtest)
