...
- **Искривлённые дедлайны:** Преобразование распределения (экспоненциальное->Вейбулл, параметр формы k=3) уменьшает длинные блоки, улучшает среднее время блоков
...
**Реализация:** `src/pocx/consensus/difficulty.cpp:GetNextGenerationSignature()` (вызывается из `src/pocx/mining/block_context.cpp:GetNewBlockContext()`)
...
- Целевое время блока: 120 секунд (все сети)
...
**Цель:** Преобразует экспоненциальное распределение в распределение Вейбулла (параметр формы k=3). Очень хорошие решения форджатся позже (сеть успевает просканировать диски), плохие решения улучшаются. Уменьшает длинные блоки, поддерживает среднее 120с.
...
**Получатель coinbase** (не проверяется консенсусом):

Майнер устанавливает выход coinbase для выплаты эффективному подписанту (`src/pocx/mining/block_builder.cpp:CreateCoinbaseScript()`), но это **не** проверяется консенсусом. Консенсус обеспечивает только то, что *подпись блока* создана эффективным подписантом — это проверка `bad-pocx-assignment-sig` выше. Правила `bad-pocx-coinbase` не существует; получатель coinbase выбирается майнером.
...
- Активируется после периода задержки (по умолчанию: 30 блоков; 4 в regtest)
