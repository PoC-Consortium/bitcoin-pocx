...
**Валидация временной метки блока** (`src/validation.cpp:ContextualCheckBlockHeader()`):
```cpp
...
**Основные файлы**:
- Валидация времени: `src/validation.cpp:ContextualCheckBlockHeader()`
- Константа допуска на будущее: `src/chain.h`
- Порог предупреждения: `src/node/timeoffsets.h`
