...
**Решение**: Преобразование распределения из экспоненциального в распределение Вейбулла (параметр формы k=3) с использованием кубического корня: `Y = масштаб × (X^(1/3))`.
...
- **Время блока**: 120 секунд (все сети: mainnet, testnet, regtest)
