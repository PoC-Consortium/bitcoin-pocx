...
- **Режим сервера майнинга**: Флаг `` включает функции, связанные с майнингом
...
**Панель инструментов**: Иконка майнинга (видна только с флагом ``)
...
- Выход OP_RETURN: маркер `POCX` + plot_address + forging_address (46-байтовый скрипт, 44-байтовая полезная нагрузка данных)
...
**Структура транзакции**:
- Вход: UTXO от адреса графика (подтверждает владение)
- Выход OP_RETURN: маркер `XCOP` + plot_address (26-байтовый скрипт, 24-байтовая полезная нагрузка данных)
- Выход сдачи: Возвращается в кошелёк
...
2. **Создание делегирования форджинга**:
   ...
   - Дождитесь задержки активации (30 blocks mainnet/testnet))
...
2. Дождаться задержки отзыва (720 blocks mainnet/testnet))
...
**Причина**: Узел запущен без флага ``
**Решение**: Перезапустить с `bitcoin-qt -server`
