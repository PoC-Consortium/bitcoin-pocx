...
- Regtest (120с): Использует режим калибровки для малой ёмкости (base_power 2^58)
...
- **Алгоритм**: Взвешенное скользящее среднее (в стиле Burstcoin, ограничение ±20% на блок)
...
**Время блоков**:
- **Целевое время блока**: `120` секунд (майнинг по запросу через `generatetoaddress` в regtest)
...
- **Калибровка малой ёмкости**: `true` (использует калибровку на 64 нонса ≈ 16 MiB вместо 1 ТиБ)

**Задержки делегирования**:
- **Активация**: `4` блока (~8 минут при интервале 120с)
- **Отзыв**: `8` блоков (~16 минут при интервале 120с)
...
- Взвешенное скользящее среднее недавних времён блоков (в стиле Burstcoin)
- Корректировка каждый блок
...
**Реализация**: `src/pocx/consensus/params.h:GetPoCXCompressionBounds()`, `src/pocx/consensus/params.cpp`
...
**Статус**: Активна (основная сеть запущена с 3 мая 2026)

**DNS Seeds основной сети**:
- `seeds.bitcoin-pocx.org`
- `bitcoin-pocx.bootseed.net`

**DNS Seeds тестовой сети**:
- `testnet.seeds.bitcoin-pocx.org`

**Конфигурация**:
- Фиксированные seed-узлы поставляются в `src/pocx/pocx_seeds.h` (формат BIP155)
- Также поддерживается ручное подключение пиров через `-addnode`
...
**Границы сжатия**: `src/pocx/consensus/params.h`, `src/pocx/consensus/params.cpp`
**Вычисление базовой цели генезиса**: `src/pocx/consensus/params.cpp`
**Логика платежа Coinbase**: `src/pocx/mining/block_builder.cpp:BuildBlock()`
