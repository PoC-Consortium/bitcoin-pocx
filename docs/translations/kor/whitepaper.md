...
(2) 데드라인 분포를 지수 분포에서 Weibull 분포(형상 k=3)로 변환하여 평균을 변경하지 않고 블록 시간 분산을 줄이는 시간 왜곡 알고리즘;
...
Where `account_id` is the 20-byte HASH160 of the miner\'s public key. 이는 암호학적으로 강력한, 채굴자 의존적 엔트로피 값의 시퀀스를 생성합니다. 채굴자의 공개키는 이전 블록이 게시될 때까지 알려지지 않으므로, 어떤 참여자도 미래의 스쿱 선택을 예측할 수 없습니다. 이는 선택적 사전 계산이나 전략적 플로팅을 방지하고 모든 블록이 진정으로 신선한 채굴 작업을 도입하도록 보장합니다.
...
| 기간 | 년차 | 반감 횟수 | 최소 스케일링 | 플롯 작업 (POC2 기준선 대비) |
|------|------|----------|--------------|---------------|
| 에포크 0 | 0-4 | 0 | X1 | 2× POC2 |
| 에포크 1 | 4-12 | 1-2 | X2 | 4× POC2 |
| 에포크 2 | 12-28 | 3-6 | X3 | 8× POC2 |
| 에포크 3 | 28-60 | 7-14 | X4 | 16× POC2 |
| 에포크 4 | 60-124 | 15-30 | X5 | 32× POC2 |
| 에포크 5 | 124+ | 31+ | X6 | 64× POC2 |

배수 열은 강화되지 않은 **POC2** 기준선을 기준으로 표현됩니다. 강화된 X1 형식이 이미 POC2 작업의 2배를 내장하고 있으므로, 레벨 Xn은 2ⁿ × POC2와 같습니다 — 동등하게 2^(n-1) × X1이며, 3.5절의 레벨별 정의와 일치합니다.
...
시스템은 2026년 5월 3일부터 메인넷에서 가동되고 있습니다. 채굴력은 해시율이 아닌 스토리지 용량에서 파생되어, 비트코인의 검증된 경제 모델을 유지하면서 에너지 소비를 수십 배 줄입니다.
