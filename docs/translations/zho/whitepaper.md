...
(2) 时间弯曲算法，将截止时间分布从指数分布转换为 Weibull 分布（形状 k=3），在不改变均值的情况下减少区块时间方差；
(3) 基于 OP_RETURN 的锻造委派机制，实现非托管矿池挖矿；以及
(4) 动态压缩扩展，根据减半计划增加绘图生成难度，以随着硬件改进保持长期安全边际。

Bitcoin-PoCX 通过最小化、特性标记的修改保持 Bitcoin Core 的架构，将 PoC 逻辑与现有共识代码隔离。该系统通过目标 120 秒区块间隔和将区块奖励调整为 10 BTCX 来保持 Bitcoin 的货币政策。减少的奖励抵消了五倍的区块频率增加，使长期发行率与 Bitcoin 的原始计划保持一致，并保持约 2100 万的最大供应量。
...
Where `account_id` is the 20-byte HASH160 of the miner\'s public key. 这产生了一系列加密强度高、依赖矿工的熵值。因为矿工的account ID在前一个区块发布之前是未知的，没有参与者可以预测未来的 scoop 选择。这防止了选择性预计算或策略性绘图，并确保每个区块都引入真正新鲜的挖矿工作。
...
| 时期   | 年份   | 减半次数 | 最低扩展 | 绘图工作（相对于 POC2 基线） |
|--------|--------|----------|----------|--------------|
| 时期 0 | 0-4    | 0        | X1       | 2 倍 POC2    |
| 时期 1 | 4-12   | 1-2      | X2       | 4 倍 POC2    |
| 时期 2 | 12-28  | 3-6      | X3       | 8 倍 POC2    |
| 时期 3 | 28-60  | 7-14     | X4       | 16 倍 POC2   |
| 时期 4 | 60-124 | 15-30    | X5       | 32 倍 POC2   |
| 时期 5 | 124+   | 31+      | X6       | 64 倍 POC2   |

倍数列相对于未加固的 **POC2** 基线格式表示。由于加固的 X1 格式已经嵌入 POC2 的 2 倍工作量，级别 Xn 等于 2ⁿ × POC2——等价于 2^(n-1) × X1，与第 3.5 节的每级定义一致。
...
| 委派激活     | 4 个区块               |
| 委派撤销     | 8 个区块               |
| 低容量模式   | 已启用（约 16 MiB 绘图，64 nonce） |
...
该系统自 2026年5月3日起已在主网上线。挖矿能力来自存储容量而非哈希率，将能源消耗降低数个数量级，同时保持 Bitcoin 经过验证的经济模型。
